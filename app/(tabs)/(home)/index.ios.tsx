...
export default function RosterScreen() {
  ...
  const handleEditDate = (date: any) => {
    console.log('[Home iOS] User tapped Edit Date button for:', date.id);
  ...
  const handleSaveEdit = async () => {
    ...
    try {
      console.log('[Home iOS] Saving date edits');
      await updateDate({
        ...selectedDate,
      ...
      console.error('[Home iOS] Error saving date edits:', error);
      Alert.alert('Error', 'Failed to update date. Please try again.');
    }
  };

  const handleRateDate = (date: any) => {
    console.log('[Home iOS] User tapped Rate Date button for:', date.id);
  ...
  const handleSaveRating = async () => {
    ...
    try {
      console.log('[Home iOS] Saving rating:', { rating, wouldGoAgain, notes: ratingNotes });
                  ...
                  <View style={styles.starsContainer}>
                    {[1, 2, 3, 4, 5].map((star) => (
                      <TouchableOpacity
                        key={star}
                        onPress={() => {
                          console.log('[Home iOS] User selected rating:', star);
                          setRating(star);
                        }}
                      >
                        <IconSymbol
