...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const signInWithGoogle = async () => {
    try {
      ...
      if (Platform.OS === "web") {
        ...
        console.log('[Auth] Using web popup flow for Google');
        const token = await openOAuthPopup("google");
        storeWebBearerToken(token);
        await fetchUser();
      } else {
        // Native: Use deep linking (handled by Better Auth)
        console.log('[Auth] Using native deep linking for Google');
  ...
  const signInWithApple = async () => {
    try {
      ...
      if (Platform.OS === "web") {
        // Web: Use popup flow
        console.log('[Auth] Using web popup flow for Apple');
        const token = await openOAuthPopup("apple");
        storeWebBearerToken(token);
        await fetchUser();
      } else {
        // Native: Use deep linking
        console.log('[Auth] Using native deep linking for Apple');
        await authClient.signIn.social({
          provider: "apple",
          callbackURL: "/(tabs)/roster",
        });
  ...
  const signInWithGitHub = async () => {
    try {
      ...
      if (Platform.OS === "web") {
        // Web: Use popup flow
        console.log('[Auth] Using web popup flow for GitHub');
        const token = await openOAuthPopup("github");
        storeWebBearerToken(token);
        await fetchUser();
      } else {
        // Native: Use deep linking
        console.log('[Auth] Using native deep linking for GitHub');
        await authClient.signIn.social({
          provider: "github",
          callbackURL: "/(tabs)/roster",
        });
