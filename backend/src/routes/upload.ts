...
export function registerUploadRoutes(app: App, fastify: FastifyInstance) {
  ...
  fastify.post(
    ...
    {
      schema: {
        description: 'Upload a roster image',
        tags: ['upload'],
        consumes: ['multipart/form-data'],
        response: { 200: { type: 'object', properties: { url: { type: 'string' }, key: { type: 'string' } } } },
      },
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      ...
      if (!data) {
        app.logger.warn({ userId: session.user.id }, 'Roster image upload: no file provided');
        return reply.status(400).send({ error: 'No file provided' });
      }
      ...
      try {
        ...
        app.logger.warn({ userId: session.user.id, filename: data.filename }, 'Roster image upload: file size too large');
        // File size limit was exceeded
        return reply.status(413).send({ error: 'File too large (max 5MB)' });
      }
      ...
      const mimeType = data.mimetype;
      if (!mimeType.startsWith('image/')) {
        app.logger.warn({ userId: session.user.id, mimeType }, 'Roster image upload: invalid file type');
        return reply.status(400).send({ error: 'File must be an image' });
      }

      const fileExtension = data.filename.split('.').pop() || 'jpg';
      const key = `uploads/roster-images/${session.user.id}/${Date.now()}.${fileExtension}`;

      try {
        app.logger.debug({ userId: session.user.id, key, filename: data.filename }, 'Uploading roster image to storage');
        const uploadedKey = await app.storage.upload(key, buffer);
        const { url } = await app.storage.getSignedUrl(uploadedKey);

        app.logger.info({ userId: session.user.id, key: uploadedKey }, 'Roster image uploaded successfully');
        return { url, key: uploadedKey };
      } catch (error) {
        app.logger.error({ err: error, userId: session.user.id, key }, 'Failed to upload roster image');
        return reply.status(500).send({ error: 'Failed to upload image' });
      }
    }
  );
